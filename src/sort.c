...
/******************************************************
 * HEAP SORT private implementation.
 ******************************************************/
void _heap_sort_sink(int* array, int size, int index){
  while (2*index+1 <= size){
    int temp = 2*index + 1;
    if (temp < size && (array[temp] < array[temp+1])){
      temp++;
    }
    if (array[temp] < array[index]) break;
    exchange(array, index, temp);
    index = temp;
  }
}
...
void heap_sort(int* array, int length){
  ...
  for(int k = (heap_length-1)/2; k >= 0; k--){
    _heap_sort_sink(array, heap_length, k);
  }

  while (heap_length >= 0){
    exchange(array, 0, heap_length);
    heap_length -= 1;
    _heap_sort_sink(array, heap_length, 0);
  }
}
